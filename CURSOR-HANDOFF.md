...
**PHP 筆記 `card-badge`／`category-tag`（Lesson 2 流程控制）：**純講 if／switch／迴圈概念者（如 `02-select`、`03-forloop`）用 **流程控制**；與傳值、專題、表單等**混合**的實戰／進階頁（如 `04-pra01`～`09-pra06`、`h02`～`h06` 主篇）用 **實戰**。
